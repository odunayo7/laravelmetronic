        ...
        <div class="col-md-12">
            @can('time_project_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.time-projects.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $timeProject->id }}">
                                        ...
                                        <td>
                                            @can('time_project_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.time-projects.show', $timeProject->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('time_project_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.time-projects.edit', $timeProject->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('time_project_delete')
                                                <form action="{{ route('frontend.time-projects.destroy', $timeProject->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('time_project_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.time-projects.massDestroy') }}",
