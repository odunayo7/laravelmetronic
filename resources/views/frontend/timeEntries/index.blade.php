        ...
        <div class="col-md-12">
            @can('time_entry_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.time-entries.create') }}">
                            ...
                            <tbody>
                                @foreach($timeEntries as $key => $timeEntry)
                                    <tr data-entry-id="{{ $timeEntry->id }}">
                                        ...
                                        <td>
                                            @can('time_entry_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.time-entries.show', $timeEntry->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('time_entry_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.time-entries.edit', $timeEntry->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('time_entry_delete')
                                                <form action="{{ route('frontend.time-entries.destroy', $timeEntry->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('time_entry_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.time-entries.massDestroy') }}",
