...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.question.title_singular') }}">
        <form method="POST" action="{{ route("admin.questions.update", [$question->id]) }}" enctype="multipart/form-data">
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('points') ? 'is-invalid' : '' }}"
                    type="number" name="points" id="points" value="{{ old('points', $question->points) }}" step="1">
