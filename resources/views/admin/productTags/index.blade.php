    ...
    @can('product_tag_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                ...
                @include('csvImport.modal', ['model' => 'ProductTag', 'route' => 'admin.product-tags.parseCsvImport'])
            </div>
        </div>
    @endcan
    <x-metronic.card flush="true" title="{{ trans('cruds.productTag.title_singular') }} {{ trans('global.list') }}">
                        ...
                        <tr data-entry-id="{{ $productTag->id }}">
                            <td>
                                <div class="form-check form-check-sm form-check-custom form-check-solid">
                                    <input class="form-check-input" type="checkbox" value="{{ $productTag->id }}" />
                                </div>
                            </td>
                            <td>
                                {{ $productTag->id ?? '' }}
                            ...
                            <td>
                                ...
                                @can('product_tag_delete')
                                    <form action="{{ route('admin.product-tags.destroy', $productTag->id) }}" method="POST"
