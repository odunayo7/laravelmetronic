...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.incomeSource.title_singular') }}">
        <form method="POST" action="{{ route("admin.income-sources.update", [$incomeSource->id]) }}"
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                    name="name" id="name" value="{{ old('name', $incomeSource->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('fee_percent') ? 'is-invalid' : '' }}"
                    type="number" name="fee_percent" id="fee_percent"
                    value="{{ old('fee_percent', $incomeSource->fee_percent) }}" step="0.01">
