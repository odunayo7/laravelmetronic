...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.test.title_singular') }}">
        <form method="POST" action="{{ route("admin.tests.update", [$test->id]) }}" enctype="multipart/form-data">
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text"
                    name="title" id="title" value="{{ old('title', $test->title) }}">
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="fv-row mb-7">
                ...
                <textarea class="form-control form-control-solid {{ $errors->has('description') ? 'is-invalid' : '' }}"
                    name="description" id="description">{{ old('description', $test->description) }}</textarea>
