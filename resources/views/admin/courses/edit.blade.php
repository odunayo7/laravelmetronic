...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.course.title_singular') }}">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="fv-row mb-7">
                <label class="required fs-6 fw-semibold mb-2"
                    for="teacher_id">{{ trans('cruds.course.fields.teacher') }}</label>
                <select class="form-control form-control-solid select2 {{ $errors->has('teacher') ? 'is-invalid' : '' }}"
                ...
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.teacher_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <label class="required fs-6 fw-semibold mb-2" for="title">{{ trans('cruds.course.fields.title') }}</label>
                <input class="form-control form-control-solid {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text"
                    name="title" id="title" value="{{ old('title', $course->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.title_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <label class="required fs-6 fw-semibold mb-2"
                ...
                <textarea class="form-control form-control-solid {{ $errors->has('description') ? 'is-invalid' : '' }}"
                    name="description" id="description" required>{{ old('description', $course->description) }}</textarea>
                @if($errors->has('description'))
                    <div class="invalid-feedback">
                        {{ $errors->first('description') }}
                    </div>
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.description_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <label class="fs-6 fw-semibold mb-2" for="price">{{ trans('cruds.course.fields.price') }}</label>
                ...
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.price_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <label class="fs-6 fw-semibold mb-2" for="thumbnail">{{ trans('cruds.course.fields.thumbnail') }}</label>
                ...
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.thumbnail_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <div
                    ...
                    <label class="form-check-label"
                        for="is_published">{{ trans('cruds.course.fields.is_published') }}</label>
                </div>
                ...
                @endif
                <div class="text-muted fs-7">{{ trans('cruds.course.fields.is_published_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
                <label class="fs-6 fw-semibold mb-2" for="students">{{ trans('cruds.course.fields.students') }}</label>
            ...
            <select class="form-control form-control-solid select2 {{ $errors->has('students') ? 'is-invalid' : '' }}"
            ...
            @endif
            <div class="text-muted fs-7">{{ trans('cruds.course.fields.students_helper') }}</div>
            </div>
            <div class="fv-row mb-7">
