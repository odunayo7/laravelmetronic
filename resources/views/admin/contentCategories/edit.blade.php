...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.contentCategory.title_singular') }}">
        <form method="POST" action="{{ route("admin.content-categories.update", [$contentCategory->id]) }}"
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                    name="name" id="name" value="{{ old('name', $contentCategory->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text"
                    name="slug" id="slug" value="{{ old('slug', $contentCategory->slug) }}">
                @if($errors->has('slug'))
                    <div class="invalid-feedback">
                        {{ $errors->first('slug') }}
                    </div>
