    ...
    @can('test_result_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                ...
                @include('csvImport.modal', ['model' => 'TestResult', 'route' => 'admin.test-results.parseCsvImport'])
            </div>
        </div>
    @endcan
    <x-metronic.card flush="true" title="{{ trans('cruds.testResult.title_singular') }} {{ trans('global.list') }}">
                        ...
                        <tr data-entry-id="{{ $testResult->id }}">
                            <td>
                                <div class="form-check form-check-sm form-check-custom form-check-solid">
                                    <input class="form-check-input" type="checkbox" value="{{ $testResult->id }}" />
                                </div>
                            </td>
                            <td>
                                {{ $testResult->id ?? '' }}
                            ...
                            <td>
                                @can('test_result_show')
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.test-results.show', $testResult->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('test_result_delete')
                                    <form action="{{ route('admin.test-results.destroy', $testResult->id) }}" method="POST"
