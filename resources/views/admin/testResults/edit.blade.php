...
@section('content')
    ...
    <x-metronic.card flush="true" title="{{ trans('global.edit') }} {{ trans('cruds.testResult.title_singular') }}">
        <form method="POST" action="{{ route("admin.test-results.update", [$testResult->id]) }}"
            ...
            <div class="fv-row mb-7">
                ...
                <input class="form-control form-control-solid {{ $errors->has('score') ? 'is-invalid' : '' }}" type="number"
                    name="score" id="score" value="{{ old('score', $testResult->score) }}" step="1">
                @if($errors->has('score'))
                    <div class="invalid-feedback">
                        {{ $errors->first('score') }}
                    </div>
